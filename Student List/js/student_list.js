...
function LoadTableArea() {
  ...
  MyStudent.StudentList().forEach(function (arrayItem) {
    ...
    cell5.innerHTML =
      "<button type='button' class='btn btn-success btn-xs' onclick='OnOff_Fn(" +
      arrayItem.id +
      ")'>On/Off</button>";

    cell6.setAttribute("class", "text-center");
    cell6.innerHTML =
      "<button type='button' class='btn btn-danger btn-xs' onclick='Delete_Fn(" +
      arrayItem.id +
      ")'>Delete</button>";

    cell7.setAttribute("class", "text-center");
    cell7.innerHTML =
      "<button type='button' class='btn btn-success btn-xs' onclick='Edit_Fn(" +
      arrayItem.id +
      ")'>Edit</button>";
  });
}
